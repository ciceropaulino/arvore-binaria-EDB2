...
public class BinarySearchTree {
    ...
    public void print(int type) {
        auxiliarPrint(root, type);
    }

    public static void auxiliarPrint(Node root, int type) {
        if(type == 1){
    ...
    private static void barDiagram(Node root, String prefix) {
        if (root == null) {
            return;
        }
    
        System.out.println(prefix + root.data + " " + getLine(root));
    
        if (root.left != null || root.right != null) {
            barDiagram(root.left, prefix + "               ");
            barDiagram(root.right, prefix + "               ");
        }
    }

    private static String getLine(Node node) {
